...
if (isset($_POST['submitPost'])) {
    if (!empty($_POST['postNew']) &&  (empty($_POST['tagNew']) && !empty($_POST['selectTagPost']))) {
        ...
        $postInsert->execute() or die(print_r($mysqli->error));
        $postInsert->close();
        ...
        $postsTagsInsert->execute() or die(print_r($mysqli->error));
    ...
    <?php
        ...
        $tagInsert->execute() or die(print_r($mysqli->error));
        $tagInsert->close();
        //insert corresponding entry for posts_tags
        $lastPostId = $mysqli->query("SELECT id FROM posts ORDER BY id DESC LIMIT 1;");
        $lastPostIdVar = $lastPostId->fetch_assoc();
...
<body>
    ...
    <div id="wrapper">
        ...
        <main>
            ...
            ";
            $postsUsers = $mysqli->query($reqPostsUsers);
            if (!$postsUsers) {
                echo ("Échec de la requete : " . $mysqli->error);
            }
            while ($post = $postsUsers->fetch_assoc()) {
            ?>
                <article>
                    <h3>
